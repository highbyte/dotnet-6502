...
| [CLI arguments](cli-arguments.md) | Drive the emulator from the command line — autostart a system, load a `.prg`, configure logging, debug, scripting, and remote control flags. | Avalonia Desktop, Headless |
...
- **"I want to launch the emulator with specific options once."** — [CLI arguments](cli-arguments.md). Compose with the above (e.g. `--script foo.lua` + `--remote-port 6510`).
