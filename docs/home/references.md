...
Used during development to compile actual 6502 source code to a binary, and then run it through the emulator.
...
Used during development to verify behavior of the 6502 CPU and C64.
...
- [Dag Lem](https://github.com/daglem) and the SID emulation community for the [reSID](https://github.com/libsidplayfp/resid) project. The chip-measured constants used by the C64 sample-based SID emulator (`SidSampleCore.cs`) — the 16-entry ADSR rate-counter table, the exponential-decay divider thresholds, the noise LFSR feedback taps, and the LFSR reset value `0x7FFFF8` — originate from reSID's measurement work. No reSID code was read or copied; only the published numeric constants are used as facts about the 6581 chip.
