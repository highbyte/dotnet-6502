...
- [`keyboard-layout-mapping-fix.md`](../../keyboard-layout-mapping-fix.md) — internal design doc
  covering the keyboard pipeline, the per-host stage-① implementations, the macOS ISO swap, and
  the auto-detect resolution chain (also documents a proposed *positional* mapping mode that is
  not yet implemented).
