...
- `test-program.asm` - Source code (ca65 assembler)
- `test-program.dbg` - Source debug info (generated by ca65 assembler)
- Optional: `build-test-program.ps1` - Build script for manual build - not required if using built-in ca65 build task in .vscode/tasks.json
- Optional: `build-test-program.sh1` - Build script for manual build -not required if using built-in ca65 build task in .vscode/tasks.json

Simple ACME assembler example
- `test-program2.prg` - 6502 program to debug
- `test-program2.asm` - Source code (ca65 assembler)
- `build-test-program2.ps1` - Build script for manual build - required because no built-in ACME assembler build task
- `build-test-program2.sh1` - Build script for manual build - required because no built-in ACME assembler build task

Full ca65 assembler C64 example (build with build task)
- `smooth_scroller_and_raster2.prg` - 6502 program to debug
- `smooth_scroller_and_raster2.asm` - Source code (ca65 assembler)
- `smooth_scroller_and_raster2.dbg` - Source debug info (generated by ca65 assembler)
