...
var commands = new[]
{
    ...
    ("oric.type",             "--text <string>",                       "Type text through the Oric keyboard latch (Oric only)"),
    ("oric.isbasicstarted",   "",                                      "Check if Oric BASIC has finished initializing (Oric only)"),
    ("oric.getbasicsource",   "",                                      "Get the current Oric BASIC program as text (Oric only)"),
    ("oric.loadtap",          "--file <path.tap> | --data <base64>",   "Direct-load the first file in a TAP image, honoring autorun (Oric only)"),
    ("oric.inserttape",       "--file <path.tap> | --data <base64>",   "Insert and rewind a TAP image for CLOAD (Oric only)"),
    ("oric.rewindtape",       "",                                      "Rewind the inserted TAP image (Oric only)"),
    ("oric.ejecttape",        "",                                      "Eject the inserted TAP image (Oric only)"),
    ("oric.tapestatus",       "",                                      "Show inserted TAP position and file metadata (Oric only)"),
