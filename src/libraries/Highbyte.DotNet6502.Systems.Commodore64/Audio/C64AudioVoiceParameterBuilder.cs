...
/// <summary>
/// Decodes C64 SID register state into a system-agnostic <see cref="AudioVoiceParameter"/> — the
/// genuinely C64-specific half of the command-stream audio source. The resulting parameter is in
/// host-neutral units (frequency in Hz, ADSR in seconds, normalized pulse width), so it can be
/// consumed by any host <see cref="IAudioCommandTarget"/>.
/// </summary>
public static class C64AudioVoiceParameterBuilder
{
        ...
        )
    {
        ...
        // ----------
        // Map SID register values to host-neutral synth parameters, and what to do with the audio.
        // ----------
        var audioVoiceParameter = new AudioVoiceParameter
        {
            ...
            OscillatorType = sidState.GetWaveForm(voice).ToAudioOscillatorType(),
