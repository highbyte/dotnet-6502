...
[HelpText("Renders C64 screen to a SkiaSharp SKCanvas using a GPU shader for compositing.\nSupports text and bitmap modes, colors per raster line, fine scroll and sprites.")]
public class C64LegacyRenderTarget2 : ICustomRenderTarget<PayloadC64>, IDisposable
{
    public string Name => "C64LegacyRenderTarget2";
    ...
    public C64LegacyRenderTarget2(C64 c64, Func<SKCanvas> getCanvas, bool flush)
    {
        _c64 = c64;
    ...
    {
        var assembly = typeof(C64LegacyRenderTarget2).Assembly;
        var resourcePrefix = $"{assembly.GetName().Name}.Resources.Shaders";
        var resourceName = $"{resourcePrefix}.{shaderFileName}";
    ...
    private void DrawBorderAndScreenToBitmapBackedByPixelArray(C64 c64, uint[] pixelArray)
    {
        ...
        for (ushort drawLine = 0; drawLine < drawableAreaHeight; drawLine++)
        {
            ...
            var colorRamAddress = (ushort)(Vic2Addr.COLOR_RAM_START + characterRow * vic2ScreenTextCols);
            var c64BitMapAddress = (ushort)(vic2BitmapBaseAddress + characterRow * vic2ScreenTextCols * 8 + characterLine);
