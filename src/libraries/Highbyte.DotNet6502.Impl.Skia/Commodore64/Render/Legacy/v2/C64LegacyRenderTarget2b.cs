...
namespace Highbyte.DotNet6502.Impl.Skia.Commodore64.Render.Legacy.v2;
...
/// </summary>
[DisplayName("Skia legacy v2b")]
[HelpText("Renders C64 graphics to a SkiaSharp SKCanvas using a GPU shader for compositing.\nGenerates text and bitmap graphics after each instruction, and sprites once per frame.")]
public class C64LegacyRenderTarget2b : ICustomRenderTarget<PayloadC64>, IDisposable
{
    public string Name => "C64LegacyRenderTarget2b";
    ...
    public C64LegacyRenderTarget2b(C64 c64, Func<SKCanvas> getCanvas, bool flush)
    {
        _c64 = c64;
        _getSkCanvas = getCanvas;
        _flush = flush;

        Init();
    }
    public ValueTask PresentAsync(PayloadC64 payloadC64, CancellationToken ct = default)
    {
        GenerateFrame();
        DrawFrame();
    ...
    private void GenerateFrame()
    {
        ...
        DrawSpritesToBitmapBackedByPixelArray(_c64, _skiaPixelArrayBitmap_BackgroundAndBorder.PixelArray, _skiaPixelArrayBitmap_Foreground.PixelArray);

        // "Draw" line data (color values of VIC2 registers per raster line) to separate bitmap (currently only used for sprites).
        DrawLineDataToBitmapBackedByPixelArray(_c64, _skiaPixelArrayBitmap_LineData.PixelArray);
    }

    private void DrawFrame()
    {        // Draw to a canvas using a shader with texture info from screen and sprite bitmaps, together with line data bitmap
        WriteBitmapToCanvas(_skiaPixelArrayBitmap_BackgroundAndBorder.Bitmap, _skiaPixelArrayBitmap_Foreground.Bitmap, _skiaPixelArrayBitmap_LineData.Bitmap, _getSkCanvas(), _c64);
    }
