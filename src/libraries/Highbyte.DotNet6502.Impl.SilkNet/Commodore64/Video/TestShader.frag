...
{ 
  SpriteData uSpriteData[8 + 8]; // 8 sprites. Add 8 to align with 16 items.
};

// Note: need total 16 bytes for each uniform value. Either a vec4, or a custom struct that adds up to 16 items.
layout (std140) uniform ubTextData
{ 
  TextData uTextData[1000 + 24];    // Text screen is 40*25 = 1000 characters. Add 24 to align with 16 items.
};
...
{ 
  ScreenLineData uScreenLineData[312 + 8]; // 312 = maximum line used by any VIC2 chip (PAL?). Add 8 to align with 16 items.
};
layout (std140) uniform ubSpriteContentData
{ 
  SpriteContentData uSpriteContentData[(8*3*21) + 8];   // 3 items (24 pixels) per row * 21 rows * 8 sprites = 63 items * 8 sprites = 504 items. Add 8 to align with 16 items.
};
...
//  BitmapData uBitmapData[1000 + 24];  // 1000 "characters" as bitmap. Add 24 to align with 16 items.
//};
//
...
bool GetTextModePixelColor(uint x, uint y, out bool border, out vec4 pixelColor)
{
    ...
    if((x < (uint(uTextScreenStart.x)+horizontalBorderOffset) || (x > (uint(uTextScreenEnd.x)-horizontalBorderOffset)))
         || (y < (uint(uTextScreenStart.y)+verticalBorderOffset) || (y > (uint(uTextScreenEnd.y)-verticalBorderOffset))) )
    {
        border = true;
        return false;
    }
	...
	uint screeny = y - uint(int(uTextScreenStart.y) + uScreenLineData[y].scrollY);
...
bool ShouldSpriteBeDrawn(int i, bool prioOverForground)
{
    // Only process visible sprites.
    // And only process sprites that matches the prioOverForground input parameter.
    if(uSpriteData[i].Visible == 0u
        && (uSpriteData[i].priorityOverForeground == 1u && !prioOverForground)
        && (uSpriteData[i].priorityOverForeground == 0u && prioOverForground))
        return false;
    return true;
}

bool IsPixelWithinSpriteArea(int i, int xi, int yi, int spriteX, int spriteY, int scaleFactorX, int scaleFactorY)
{
...
bool GetSpritePixelColorLAB(uint x, uint y, bool prioOverForground, out vec4 spritePixelColor)
{
    // TEST: set color to debug if true is returned but pixelColor not set by code below.
    spritePixelColor = vec4(0,1,1,1);
    ...
    // TEMP TEST
    if(uSpriteData[0].Visible == 0u
        && uSpriteData[1].Visible == 0u
        && uSpriteData[2].Visible == 0u
        && uSpriteData[3].Visible == 0u
        && uSpriteData[4].Visible == 0u
        && uSpriteData[5].Visible == 0u
        && uSpriteData[6].Visible == 0u
        && uSpriteData[7].Visible == 0u
       )
    {
        //spritePixelColor = vec4(1,0,1,1);
        return false;
    }
    //    spritePixelColor = vec4(1,0,0,1);
    //    return true;
    // END TEMP TEST

    // Sprite/sprite priority is first sprite 0 (will be drawn over others), then 1, etc to 7. 
    bool pixelIsSet = false;
    int xi = int(x);
    int yi = int(y);
    for(int i=0; i<8; i++)
    {
        // Check if sprite is visible and has correct priority (for being drawn now)
        bool shouldSpriteBeDrawn = ShouldSpriteBeDrawn(i, prioOverForground);
        if(shouldSpriteBeDrawn)
        {
            // Check if pixel in within sprite screen area
            ...
            bool isPixelWithinSpriteArea = IsPixelWithinSpriteArea(i, xi, yi, spriteX, spriteY, scaleFactorX, scaleFactorY);
            if(isPixelWithinSpriteArea)
            {
                ...
                if(!(dx<0 || dy<0))
                {
                    ...
                    if(uSpriteData[i].multiColor == 0u)
                    {
                        ...
                        if(foreground)
                        {
                            ...
                            spritePixelColor = uColorMapData[colorCode & 15u].color;
                            pixelIsSet = true;
                            break;
                        }
...
void Test()
{
    if(uDisplayMode == 999 || uBitmapMode == 999 || uTextCharacterMode == 999)
        return;
...
//    uint x = uint(float(gl_FragCoord.x) * 1.0f/uScale.x);
//    uint y = uint((uWindowSize.y - float(gl_FragCoord.y)) * 1.0f/uScale.y); // Make sure top/left is 0,0
//
//    uint horizontalBorderOffset = 0u;
//    uint verticalBorderOffset = 4u;
//
//    if((x < (uint(uTextScreenStart.x)+horizontalBorderOffset) || (x > (uint(uTextScreenEnd.x)-horizontalBorderOffset)))
//         || (y < (uint(uTextScreenStart.y)+verticalBorderOffset) || (y > (uint(uTextScreenEnd.y)-verticalBorderOffset))) )
//    {
...
//    {
//        // Not border
//        uint bgColorCode0 = uScre6enLineData[y].backgroundColor0Code;
//        vec4 bgColor0 = uColorMapData[bgColorCode0 & 15u].color;
//        FragColor = bgColor0;
//    }
//
...
void main()
{
    ...
    {
        finalColor = prioSpritePixelColor;
        colorIsSet = true;
    }
    else
    {
    ...
    //    }
        ...
        if(!colorIsSet)
        {
            ...
            {
                finalColor = nonPrioSpritePixelColor;
                colorIsSet = true;
            }
            else
            {
    ...
    if(!colorIsSet)
    {
        // Shouldn't happen, but set green color to indicate error
