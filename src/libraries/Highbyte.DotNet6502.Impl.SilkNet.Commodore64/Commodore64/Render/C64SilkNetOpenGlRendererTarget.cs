...
public class C64SilkNetOpenGlRendererTarget : ICustomRenderTarget<C64GpuPacket>, IDisposable
{
    ...
    private void InitShader(C64 c64)
    {
        ...
        var assemblyName = typeof(C64SilkNetOpenGlRendererTarget).Assembly.GetName().Name;
        var shaderResourcePrefix = $"{assemblyName}.Commodore64.Render";
        var vertexShaderSource = OpenGLHelpers.Shader.LoadEmbeddedShaderSource(
            $"{shaderResourcePrefix}.C64shader.vert",
            typeof(C64SilkNetOpenGlRendererTarget).Assembly);
        var fragmentShaderSource = OpenGLHelpers.Shader.LoadEmbeddedShaderSource(
            $"{shaderResourcePrefix}.C64shader.frag",
            typeof(C64SilkNetOpenGlRendererTarget).Assembly);
    ...
    public void DrawFrame(C64GpuPacket c64GpuPacket)
    {
        ...
        _shader.SetUniform("uWindowSize", new Vector3(framebufferSize.X, framebufferSize.Y, 0), skipExistCheck: true);

        var scaleX = (float)framebufferSize.X / c64GpuPacket.Vic2Screen.VisibleWidth;
        var scaleY = (float)framebufferSize.Y / c64GpuPacket.Vic2Screen.VisibleHeight;
