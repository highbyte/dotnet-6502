-- example_emulator_control_ascii.lua
-- ASCII-only clone of example_emulator_control.lua for testing published
-- browser/AOT parsing without any non-ASCII comments or strings.
--
...
while true do
    ...
    -- Write a log message every 5 seconds independent of emulator state, using
    -- wall-clock time. emu.yield() at end of loop ensures this keeps ticking
    -- while paused.
    ...
    -- Log state every 60 frames (only when running, since framecount does not
    -- advance while paused).
    if emu.state() == "running" and frame % 60 == 0 then
        log.debug(string.format("Frame %d | state=%s", frame, emu.state()))
    end

    -- Use emu.yield() so this script keeps ticking even while paused, allowing
    -- it to observe the paused state and call emu.start().
    emu.yield()
end
